...
async function createStore(req, res, next) {
    try {
        ...
        const { name, tagline, description, businessType, logo, cover, primaryCategory, supportingCategories, pincode, area, city, address, businessMobile, email, website, workingHours, delivery } = req.body;
        ...
        const store = await Store_1.Store.create({
            ...
            delivery: delivery || {},
            status: contracts_1.StoreStatus.SUBMITTED
        });
        await StoreMember_1.StoreMember.create({
            storeId: store._id,
            userId: ownerId,
            role: "owner",
            permissions: ["*"],
...
async function getPublicStores(req, res, next) {
    try {
        const page = Math.max(1, parseInt(req.query.page) || 1);
        const limit = Math.min(50, Math.max(1, parseInt(req.query.limit) || 20));
        const skip = (page - 1) * limit;
        const query = {
            status: { $in: [contracts_1.StoreStatus.APPROVED, contracts_1.StoreStatus.ACTIVE, contracts_1.StoreStatus.SUBMITTED, "under_review", "submitted", "active", "APPROVED", "ACTIVE"] }
        };
        if (req.query.category)
            query.primaryCategory = req.query.category;
        if (req.query.city)
...
async function getStoreById(req, res, next) {
    try {
        ...
        const store = isObjectId
            ? await Store_1.Store.findById(storeId).lean()
            : await Store_1.Store.findOne({ $or: [{ slug: storeId }, { name: new RegExp(`^${storeId}$`, "i") }] }).lean();
        if (!store) {
...
async function getMyStores(req, res, next) {
    try {
        ...
        const stores = await Store_1.Store.find({ ownerId: req.user._id }).sort({ createdAt: -1 }).lean();
        res.status(200).json({
            success: true,
            data: stores.map((s) => ({
                id: s._id.toString(),
...
async function getStoreListings(req, res, next) {
    try {
        const { storeId } = req.params;
        const page = Math.max(1, parseInt(req.query.page) || 1);
        const limit = Math.min(50, Math.max(1, parseInt(req.query.limit) || 20));
        const skip = (page - 1) * limit;
        const isObjectId = /^[0-9a-fA-F]{24}$/.test(storeId);
        const targetStore = isObjectId ? await Store_1.Store.findById(storeId).lean() : await Store_1.Store.findOne({ slug: storeId }).lean();
        const query = {
            status: { $in: [contracts_1.ListingStatus.APPROVED, contracts_1.ListingStatus.ACTIVE, contracts_1.ListingStatus.SUBMITTED, "APPROVED", "ACTIVE", "SUBMITTED", "approved", "active", "submitted", "under_review"] }
        };
        if (targetStore) {
...
async function updateStore(req, res, next) {
    try {
        ...
        if (primaryCategory)
            updateFields.primaryCategory = primaryCategory;
        if (pincode)
        ...
        if (businessMobile)
            updateFields.businessMobile = businessMobile;
        if (email)
            updateFields.email = email;
        if (website !== undefined)
            updateFields.website = website;
        const updatedStore = await Store_1.Store.findOneAndUpdate({ _id: storeId, ownerId: req.user._id }, { $set: updateFields }, { new: true, runValidators: true }).lean();
        if (!updatedStore) {
