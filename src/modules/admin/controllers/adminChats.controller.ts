...
export async function getAdminConversations(req: AuthenticatedAdminRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    ...
    if (contextType && contextType !== "ALL") {
      filter.contextType = (contextType as string).toUpperCase();
    }
    ...
    const items = rawConversations.map((c: any) => {
      const participants = (c.participantIds || []).map((p: any) => ({
        id: p._id?.toString(),
      ...
      finalItems = items.filter(
        (item: any) =>
          ...
          item.participants.some(
            (p: any) => p.name?.toLowerCase().includes(search) || p.email?.toLowerCase().includes(search)
          ) ||
...
export async function getAdminConversationMessages(req: AuthenticatedAdminRequest, res: Response, next: NextFunction): Promise<void> {
  try {
...
export async function flagAdminConversation(req: AuthenticatedAdminRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    const { conversationId } = req.params;
    ...
    (conversation as any).isFlagged = Boolean(isFlagged);
    if (flagReason) {
      (conversation as any).flagReason = flagReason;
    }
    ...
    res.status(200).json({
      ...
      data: {
        ...
        isFlagged: (conversation as any).isFlagged,
        flagReason: (conversation as any).flagReason
      }
    });
  } catch (error) {
    next(error);
...
export async function sendAdminWarningMessage(req: AuthenticatedAdminRequest, res: Response, next: NextFunction): Promise<void> {
  try {
    if (!req.admin) {
